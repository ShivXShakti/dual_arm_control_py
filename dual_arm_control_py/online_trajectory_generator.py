...
class TrajectoryGenerator():
    ...
    def FK(self, theta, dh_l, leftarm_f = False, rightarm_f = False):
        """
    ...
    def joints_trajectory(self, t, traj_time, theta, dh_l, T_init, T_final, leftarm_f, rightarm_f):
        ...
        if leftarm_f:
            theta = theta[0:7]
            q_dot_constraints = (self.mean_joints_limit[0:7].reshape(7,1)-np.array(theta).reshape(7,1))/self.max_min_dif_sq[0:7].reshape(7,1)
        elif rightarm_f:
            theta = theta[7:14]
            ...
            q_dot_constraints = np.array([0.0 for _ in range(7)])
        #### ik
