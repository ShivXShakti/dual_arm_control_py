...
class PositionCommander(Node):
    def __init__(self):
        ...
        self.T_final = np.array([[0, -1, 0, 0], 
                                 [0, 0, -1, -.10555-0.176-0.3-0.32-0.225], #0.10555,0.176,0.3,0.32,0.2251
                                 [1, 0, 0, 0], 
                                 [0, 0, 0, 1]])
        self.T_init = None
    ...
    def listener_callback(self, msg):
        ...
        if not self.js_callback:
            _, self.T_init = self.obj.FK(self.ordered_positions[:7], self.obj.dh_l)
        self.js_callback = True
    def timer_callback(self):
        if self.ordered_positions is None:
            return
        msg = Float64MultiArray()
        
        joints = self.obj.get_joints(self.t_counter, [self.ordered_positions[7], self.ordered_positions[8], self.ordered_positions[9], self.ordered_positions[10], self.ordered_positions[11], self.ordered_positions[12], self.ordered_positions[13]], self.obj.dh_l, self.T_init, self.T_final)
